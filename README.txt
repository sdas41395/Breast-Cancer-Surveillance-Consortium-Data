...
Dataset:
The public dataset chosen for this was taken from the Breast Cancer Surveillance Consortium. The dataset was taken from 2,392,998 screening mamograms and included patient outcomes from the women. These outcomes described whether these women were diagnosed with breast cancer (invasive or ductal carcinoma in situ) within one year following the index screening mammogram. The parameters of the data include age, race, biopsy results, relatives with breast cancer, bmi, birad score, among others.

To Run the Software:
The front end is coded in Shiny R so simply opening up the ui.R and the server.R in R Studio and hitting run_app() should bring up the application.
The back end is coded in python and deals with parsing the actual dataset. Everytime the submit is hit in the front end triggers the back end data parsing.
The dataset is located in ./Das_Shoham/backend/dataset
The saved_results is located in ./Das_Shoham/saved_results.txt

Parameters:
The parameters located on the right of the graphs are the constraints. Checking any of the boxes will constrain the program to select for that box when parsing. Leaving no boxes checked in any parameter is the equivalent of checking them all
